...
async function verifyTmpWritable() {
  const directory = await mkdtemp('/tmp/job-copy-preflight-');
  try {
...
export async function runPreflight({
  ...
  const inputErrors = [];
  if (!isUnderTmp(resolvedJoblist)) inputErrors.push(`Joblistは/tmp配下へ置いてください: ${resolvedJoblist}`);
  ...
  checks.push(inputErrors.length === 0
    ...
    : result(5, '入力CSV（Joblist）', 'NG', inputErrors.join(' / '), 'JoblistタブをCSVで再ダウンロードし、/tmpへ置いてください。'));
  ...
  try {
    ...
    checks.push(result(10, '/tmp書き込み', 'OK', '一時ファイルを作成・削除できました'));
  } catch (error) {
    checks.push(result(10, '/tmp書き込み', 'NG', error.message, '/tmpの権限と空き容量を確認してください。'));
  }
...
export async function runSelfTest() {
  ...
  await check('Joblist UI差異吸収', () => {
    ...
    const context = { clientId: 'foot', csvPath: '/tmp/same.csv', config, columnMap };
    assert(
  ...
  await check('/tmp書き込み', verifyTmpWritable);
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write(
      'Usage: node scripts/preflight.mjs --client <id> --joblist /tmp/file.csv\n'
