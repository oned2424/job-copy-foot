...
def _self_test():
    ...
    assert out_path("foot", "joblist", "20260802") == "/tmp/foot_joblist_20260802.csv"
    print("self-test OK")
...
def main():
    ...
    if not a.out_dir.startswith("/tmp"):
        sys.exit("NG: 出力先は /tmp にしてください（実データを Drive 配下に置かない）")
    ...
    for label, sid, sheet, rng, kind in tgt:
        ...
        with open(path, "w", encoding="utf-8", newline="") as f:
            csv.writer(f).writerows(pad(rows))
        made[kind] = path
