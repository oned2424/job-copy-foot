...
function lexicalTmpPath(candidate, kind) {
  const resolved = path.resolve(candidate);
  const relative = path.relative('/tmp', resolved);
  if (!relative || relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
    throw new Error(`${kind} must be a file under /tmp: ${resolved}`);
  }
  return { resolved, relative };
}

async function assertNoSymlinkComponents(resolved, relative, { allowMissingLeaf = false } = {}) {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    if (metadata.isSymbolicLink()) throw new Error(`Path must not contain symlinks below /tmp: ${current}`);
    ...
    if (!isPathInside(actualTmpRoot, actualCurrent)) {
      throw new Error(`Path resolves outside /tmp: ${current} -> ${actualCurrent}`);
    }
  }
  const actualResolved = await realpath(resolved);
  if (!isPathInside(actualTmpRoot, actualResolved)) {
    throw new Error(`Path resolves outside /tmp: ${resolved} -> ${actualResolved}`);
  }
}

async function secureReadTmpJson(inputPath) {
  const { resolved, relative } = lexicalTmpPath(inputPath, 'Input');
  await assertNoSymlinkComponents(resolved, relative);
  const metadata = await lstat(resolved);
  if (!metadata.isFile()) throw new Error(`Input path is not a regular file: ${resolved}`);
...
async function ensureSecureOutputParent(outputPath) {
  const { resolved, relative } = lexicalTmpPath(outputPath, 'Output');
  const parent = path.dirname(resolved);
  const parentRelative = path.relative('/tmp', parent);
  if (parentRelative) {
    let current = '/tmp';
    ...
    await assertNoSymlinkComponents(parent, parentRelative);
  }
  await assertNoSymlinkComponents(resolved, relative, { allowMissingLeaf: true });
  return resolved;
}

async function secureWriteTmpJson(outputPath, value) {
  const resolved = await ensureSecureOutputParent(outputPath);
  try {
    const metadata = await lstat(resolved);
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write([
      'Usage:',
      '  node audit_tags.mjs --input /tmp/normalized.json [--contract /tmp/contract.json] --client <id> --output /tmp/raw.json',
