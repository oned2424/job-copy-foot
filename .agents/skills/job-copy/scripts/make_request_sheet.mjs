...
function resolveTmpPath(candidate, kind, allowTmpRoot = false) {
  ...
  if (!isPathInside('/tmp', resolved, allowTmpRoot)) {
    throw new Error(`${kind} must be ${allowTmpRoot ? '/tmp or a path below /tmp' : 'a file below /tmp'}: ${resolved}`);
  }
  return resolved;
}
...
async function readJson(jsonPath, label) {
  try {
    ...
    throw new Error(`${label}を読み込めません: ${jsonPath}: ${error.message}`, { cause: error });
  }
}

async function readTmpJson(inputPath) {
  const resolved = resolveTmpPath(inputPath, 'Contract input');
  const actualTmp = await realpath('/tmp');
  const metadata = await lstat(resolved);
  if (metadata.isSymbolicLink() || !metadata.isFile()) {
    throw new Error(`Contract input must be a regular file and not a symbolic link: ${resolved}`);
  }
  const actualInput = await realpath(resolved);
  if (!isPathInside(actualTmp, actualInput, false)) {
    throw new Error(`Contract input resolves outside /tmp: ${resolved}`);
  }
  return readJson(actualInput, '内容確認書JSON');
}

async function writeTmpText(outputPath, content) {
  const resolved = resolveTmpPath(outputPath, 'Output');
  const outputDirectory = path.dirname(resolved);
  const directoryMetadata = await lstat(outputDirectory);
  const isTmpRoot = path.resolve(outputDirectory) === '/tmp';
  if ((!isTmpRoot && directoryMetadata.isSymbolicLink()) || (!directoryMetadata.isDirectory() && !isTmpRoot)) {
    throw new Error(`Output directory must be a directory and not a symbolic link: ${outputDirectory}`);
  }
  const actualTmp = await realpath('/tmp');
  const actualDirectory = await realpath(outputDirectory);
  if (!isPathInside(actualTmp, actualDirectory, true)) {
    throw new Error(`Output directory resolves outside /tmp: ${outputDirectory}`);
  }
  try {
    const existing = await lstat(resolved);
    if (existing.isSymbolicLink() || !existing.isFile()) {
      throw new Error(`Output must be a regular file and not a symbolic link: ${resolved}`);
    }
  } catch (error) {
...
async function generateRequest(options) {
  ...
  const isTmpRoot = path.resolve(outputDirectory) === '/tmp';
  if ((!isTmpRoot && metadata.isSymbolicLink()) || (!metadata.isDirectory() && !isTmpRoot)) {
    throw new Error(`Output directory must exist, be a directory, and not be a symbolic link: ${outputDirectory}`);
  }
...
async function runSelfTest() {
  ...
  const tmpDirectory = await mkdtemp('/tmp/job-copy-request-selftest-');
  try {
...
async function main() {
  ...
  if (options.selfTest) {
    if (options.client || options.contract || options.outputDir !== '/tmp') {
