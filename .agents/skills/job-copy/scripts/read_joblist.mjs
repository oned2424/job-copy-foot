...
import { TMP_ROOT, secureWriteTmpFile } from './secure_tmp.mjs';
...
export function runSelfTest() {
  ...
  const context = {
    clientId: 'foot',
    csvPath: path.join(TMP_ROOT, 'self-test.csv'),
    config,
    columnMap,
  };
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write(
      'Usage: node read_joblist.mjs --csv <path> --client <id> [--output <private-temp>/file.json]\n'
