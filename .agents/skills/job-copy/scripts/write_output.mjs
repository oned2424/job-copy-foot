...
export async function runSelfTest() {
  ...
  const tmpDirectory = await mkdtemp('/tmp/job-copy-write-output-selftest-');
  try {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write(
      'Usage: node write_output.mjs (--tag-audit /tmp/raw.json | --variants /tmp/raw.json) '
      + '--client <id> --date YYYYMMDD [--output-dir /tmp]\n'
