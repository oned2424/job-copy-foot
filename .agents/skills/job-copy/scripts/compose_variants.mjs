...
import { secureReadTmpText, secureWriteTmpFile } from './secure_tmp.mjs';
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (args.from && !args.help) {
    ...
    const outPath = String(args['plan-out'] || '/tmp/plan.json');
  ...
  const [current, siblings, request] = await Promise.all([
    readJsonFromTmp(String(args.current || '/tmp/current.json')),
    readJsonFromTmp(String(args.siblings || '/tmp/siblings.json')),
    readJsonFromTmp(String(args.request || '/tmp/request.json')),
  ]);
  ...
  const outPath = String(args.out || '/tmp/plan_candidates.json');
