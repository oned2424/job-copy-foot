...
export function runSelfTests({ clientId, config, limits, rulesDocument } = {}) {
  ...
  const results = cases.map(({ ownerRuleId, testCase }, index) => {
    ...
    return {
      ...
      ownerRuleId,
      passed: /below|inside|must be/.test(message),
      expectedRuleIds,
      actualRuleIds,
...
async function runTmpGateChecks(clientId) {
  ...
  const checks = [
    { name: 'tmp_gate_input', argv: ['--client', clientId, '--input', outside] },
    {
      ...
      argv: ['--client', clientId, '--input', path.join(TMP_ROOT, 'job-copy_gate_probe.json'),
        '--output-dir', '/etc'],
    },
  ];
  const results = [];
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write([
      'Usage:',
      '  node lint_copy.mjs --input <normalized.json> --client <id> [--output-dir <private-temp>] [--date YYYYMMDD]',
  ...
  const outputDirectory = await ensureSecureTmpDirectory(options.outputDir ?? TMP_ROOT);
