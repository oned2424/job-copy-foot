> **Windows native 互換版をご利用の場合**: この文書の WSL/Ubuntu 向け手順は配布元の原文です。Windows の Codex アプリでは、先にプロジェクト直下の WINDOWS_SETUP.md を読み、Windows native のまま進めてください。\n\n# job-copy 初期設定と実行手順
git URL：
...
手順3も、必要なURLと鍵ファイルを人が用意したあとであれば、
...
ここで4つ（＋任意の1つ）を用意し、最後にスキルへ登録します。導入時に1回だけの作業です。
...
| 2 | 内容確認書スプレッドシート | 派遣先ごとの契約条件を読む | 既にあるものをそのまま使う | 編集 |
| 3 | ヒアリング履歴スプレッドシート | 一度聞いた質問と回答を貯める | **スクリプトが作ります**（後述） | 編集 |
| 4 | 出稿用ドキュメントの保存先フォルダ | 「記載項目_派遣先名」の出力先 | ドライブで空のフォルダを新規作成する | 編集 |
| 5 | AirWorkの有料広告の実績スプレッドシート（**任意**） | どの欄を直せば応募が増えるかを数字で決める | Airワークの管理画面から「求人単位」の実績をエクスポートし、スプレッドシートとして保存する | 編集 |

**共有はすべて編集者で統一します。**権限を1つずつ変えると、
どれが閲覧者のままだったかを後から追う手間が生まれるためです。

ただし**スキルが実際に書き込むのは1・3・4だけです。**
2（内容確認書）と5（有料広告の実績）は、権限が編集者でも読むだけで、
一度も書き換えません。

**5は無くてもスキルは動きます。**無い場合は、5案の作り分けを実績ではなく
原稿の中身だけで決めます。あれば「クリックは取れているのに応募で落ちている」といった
判定ができるので、直す欄を当てずっぽうで選ばずに済みます。

#### 5を登録すると、依頼のたびにURLを聞かなくなる

**5は一度登録すれば、以後フロントは何も渡さなくて済みます。**
実績を毎回チャットに貼ってもらう運用にすると、貼り忘れた回だけ判定が消えます。

**タブは毎月足してかまいません。**スキルは登録されたファイルの中から
`sheetHint`（既定は「求人単位」）を含むタブを集め、**タブ名の年月が一番新しいもの**を選びます
（年月が読めなければ一番右のタブ）。`求人単位_202609` のように月をタブ名に入れておけば、
翌月にタブを足すだけで、設定を触らずに新しい月が読まれます。

**読んだファイル名とタブ名は実行のたびに画面に出ます。**古い月を読んでいたときに
気づけるのはここだけなので、この2行は必ず確認してください。
...
**すべて、その会社のフォルダの中に置きます。**同じフォルダにまとめる必要はありませんが、
次のように分けると迷いません。

```text
{会社名}/
├── 入力/       ← 1 Joblist・2 内容確認書・3 ヒアリング履歴・5 有料広告の実績
└── 出稿用/     ← 4 出稿用ドキュメントの保存先フォルダ
```

**入力側と出力フォルダは分けてください。**同じ場所にすると、出稿用ドキュメントが増えたときに
...
> **FooTは A（担当者のGoogleアカウント）で運用します。**
> スキルは **`foot.tominaga@gmail.com`（富永さん）** としてGoogleにアクセスします。
> 5つのファイル・フォルダは、すべてこのアドレスに**編集者**で共有済みです。
> **通知メールは送っていません。**共有されていること自体は市野から伝えます。
> → 手順は下の「**A（担当者のGoogleアカウント）を選ぶ場合**」だけを読めば足ります。
> Bの節は読み飛ばしてかまいません。
>
> Aを選んだ以上、**スキルは富永さんがドライブで見えるファイル全部に手が届く状態で動きます。**
> スキルが実際に触るのは設定した5つだけですが、権限としては届きます。
> これが困る場合はBに切り替えられます（設定ファイルの書き換えは不要で、
> 共有先をロボットのアドレスに足すだけです）。市野へ連絡してください。

#### A（担当者のGoogleアカウント）を選ぶ場合 ← FooTはこちら
...
ブラウザが開くので、**共有先に指定したアカウント**（FooTなら `foot.tominaga@gmail.com`）で
ログインします。**別のアカウントでログインすると、共有されていないので全部 `[NG]` になります。**

スプレッドシート・ドキュメント・ドライブの3つが要ります。`spreadsheets` だけで通すと、
ヒアリング履歴までは動き、**出稿用ドキュメントの作成でだけ落ちます。**

いま何のアカウントとして動いているかは、次で確認できます。

```bash
python3 scripts/init_client.py --client foot --check
```

先頭の「■ 認証」行に出ます。**ここが想定と違うアカウントなら、先へ進まないでください。**

#### B（サービスアカウント）を選ぶ場合 ← FooTでは使いません

> FooTはAで運用します。**この節は読み飛ばしてかまいません。**
> 他社で使うとき、または将来Bへ切り替えるときの手順です。
...
4. 用意したものを、そのメールアドレスに共有する

| 対象 | 共有時に選ぶ権限 |
|---|---|
| Joblistスプレッドシート | 編集者（`入稿指示ログ` タブを追加するため。`Sheet1` は書き換えません） |
| 内容確認書スプレッドシート | 編集者（スキルは読むだけ） |
| ヒアリング履歴スプレッドシート | 編集者 |
| 出稿用ドキュメントの保存先フォルダ | 編集者 |
| AirWorkの有料広告の実績スプレッドシート（任意） | 編集者（スキルは読むだけ） |

**5つとも編集者で統一します。**書き込みが必要なのは上3つと4つめですが、
権限を1つずつ変えると、どれが閲覧者のままだったかを後から追う手間が生まれます。
...
> **FooTのぶんは作成済みです。**共有も済んでいるので、この節は飛ばして
> 次の「登録内容を確かめる」へ進んでください。
> 以下は、FooT以外の会社を追加するときの手順です。
...
**FooTの5つは、すでに登録済みです。URLを貼る作業はありません。**
どこを見に行くかは `references/clients/foot/config.json` に書いてあり、
このファイルは配布物に含まれています。手で編集する必要もありません。

ここでやるのは「**登録されている5つが、自分の環境から本当に開けるか・書けるか**」の確認だけです。
...
```text
■ 認証: ユーザー: foot.tominaga@gmail.com
...
■ 疎通確認
  ...
  [OK]     AirWorkの有料広告の実績（任意） → ...
```

**見るところは2つです。**

1. **先頭の「■ 認証」行。**ここが `foot.tominaga@gmail.com` になっていること。
   別のアカウントなら、共有されていないので下がすべて `[NG]` になります。
   `gcloud auth application-default login` をやり直してください。
2. **5つとも `[OK]` になっていること。**1つでも `[NG]` なら先へ進まないでください。

`[NG]` が出たら、そのアカウントに共有が届いているかを市野へ連絡してください。
**設定ファイルを手で書き換えないでください。**次の更新配布で上書きされます。

このとき、**Joblistとヒアリング履歴は「開けるか」だけでなく「書けるか」まで確かめます。**
この2つは編集権限が要るためです（Joblistは `入稿指示ログ` タブを追加するため。
Airワークからエクスポートした `Sheet1` は書き換えません）。
内容確認書と有料広告の実績は、権限は編集者ですがスキルは読むだけです。

> **⚠ 5つめ（有料広告の実績）は、いま検証用のデータが入っています。**
> ファイル名は `FooT_AirWork有料広告実績_テストデータ`、タブは `求人単位_202608` です。
> **判定の仕組みを確かめるためのもので、この数字で出し分けの判断はできません。**
>
> 実行のたびに、次の1行が出ます。
>
> ```text
> ※ このファイルはまだ本番の実績ではありません（名前に「テスト」が入っています）。
>    AirWorkからエクスポートした実データを新しいタブとして足し、ファイル名からその語を外すと、この行は消えます
> ```
>
> **本番に切り替える手順は2つだけです。設定ファイルは触りません。**
>
> 1. AirWorkの管理画面から「求人単位」の実績をエクスポートし、
>    **同じファイルの中に `求人単位_202609` のような新しいタブとして貼る**（年月をタブ名に入れる）
> 2. **ファイル名から「テストデータ」を消す**（例：`FooT_AirWork有料広告実績`）
>
> これで上の警告は消えます。以後も**毎月タブを足すだけ**です。
> スキルは「求人単位」を含むタブのうち**年月が一番新しいもの**を自動で選ぶので、
> タブ名を聞かれることも、設定を直すこともありません。

FooT以外の会社で使い始めるときは、`--check` を付けずに実行すると5項目のURLを順に聞かれます
（`--client` には別の名前を付けます。Enterで据え置き、5つめは任意なのでEnterで飛ばせます）。
設定一式が複製され、同じ流れで登録できます。ただし複製された `question-catalog.json` には
